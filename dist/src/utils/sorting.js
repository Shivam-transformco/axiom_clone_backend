...
function applySorting(items, sortBy, sortDir, period) {
...
function getValue(item, by, period) {
    switch (by) {
        case 'volume':
        ...
        case 'priceChange':
            if (period === '1h')
                return item.price_1hr_change;
            if (period === '24h')
                return item.price_24h_change;
            if (period === '7d')
                return item.price_7d_change;
