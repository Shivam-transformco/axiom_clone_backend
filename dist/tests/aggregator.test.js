...
const aggregator_1 = require("../src/services/aggregator");
...
jest.mock('../src/datasources/jupiter', () => ({
    getJupiterPrices: async (_ids) => ({ x: 1.2, y: 2.3, z: 3.4 }),
}));
test('discover path returns gecko tokens enriched with jupiter and filtered', async () => {
    const { items } = await (0, aggregator_1.getTokens)({ limit: 10, sortBy: 'price', sortDir: 'desc', minVolume: 150 });
...
test('search path merges duplicates and prefers max volume/liquidity', async () => {
    const { items } = await (0, aggregator_1.getTokens)({ q: 'x', limit: 10, sortBy: 'volume', sortDir: 'desc' });
